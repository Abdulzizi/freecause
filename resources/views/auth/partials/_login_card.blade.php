<div class="bg-white rounded-3 shadow-sm p-4" style="border:1px solid #eee;">
    ...
    <form method="POST" action="{{ lroute('login.post') }}">
        ...
        <div class="mt-3">
            <a href="{{ lroute('register') }}" class="red d-block fw-bold">{{ trans_db('auth.not_member') }} {{ trans_db('auth.sign_up_now') }}</a>
            <a href="{{ lroute('password.request') }}" class="red d-block mt-2">{{ trans_db('auth.forgot_password') }}</a>
            <a href="{{ lroute('verification.resend.form') }}" class="red d-block">{{ trans_db('auth.resend_verification') }}</a>
        </div>
    </form>
</div>